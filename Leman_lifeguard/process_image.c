 ...
 */
uint16_t extract_swimmer_width(uint8_t *buffer_red, uint8_t *buffer_blue){ //ajout buffer blue
	...
	do{
		not_swimmer = 0;
		//begin = rising_slope(buffer); //can't use that bc need to memorize i,  or in next step take i = begin
		...
		{ 
		    ...
		    if((buffer_red[i] > mean_red) && (buffer_red[i-WIDTH_SLOPE] < mean_red)){// && (buffer_blue[i] < mean_blue) && (buffer_blue[i-WIDTH_SLOPE] > mean_blue)){
		        begin = i;
		        stop = 1;
		    }
		    i++;
		...
		//if begin = 0 : pas de pente montante, if begin !=0
...
static THD_FUNCTION(ProcessImage, arg) {
    ...
    while(1){
		...
		SwimmerWidth = extract_swimmer_width(imr, imb);
		...
		if(send_to_computer){
			...
			SendUint8ToComputer(imr, IMAGE_BUFFER_SIZE); // ausis flancs montant e vert et bleu quand voit du rouge :(
		}
		//invert the bool
